...
@section('title', 'Ubah User - Sistem Ringkasan')
...
<div class="space-y-8">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex flex-col gap-0.5">
            <h1 class="heading-font text-2xl font-extrabold text-[#0f2a4a] tracking-tight">Ubah Data User</h1>
            <p class="text-xs text-slate-500 font-medium">Ubah profil administrator atau perbarui password user</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8">
        <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-1.5">
                ...
                >
                    <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>User Umum (Akses Portal Ringkasan)</option>
                    <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Administrator (Akses Dashboard Admin)</option>
                </select>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 space-y-4">
                <div class="flex flex-col gap-0.5">
                    <h3 class="font-bold text-slate-800 text-sm">Ganti Password (Opsional)</h3>
                    <p class="text-xs text-slate-400">Kosongkan kolom di bawah jika tidak ingin merubah kata sandi user ini.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        <label for="password_confirmation" class="text-xs font-bold text-slate-600 uppercase tracking-wider block">Konfirmasi Password Baru</label>
                        <div class="relative">
                            <input 
                                type="password" 
